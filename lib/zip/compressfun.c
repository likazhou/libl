...
int PeriodCompression(DATA *buffer)
{
	...
	else//
	{
		...
		if ((MaxMinColumn = (word8 *)mem_Malloc(period * 2)) == NULL)
			exit(0);
		
		record.length = (period+7) / 8;
		
		if ((record.x = (word8 *)mem_Malloc(record.length)) == NULL)
			exit(0);
		
		memset(record.x, 0, record.length);
		
		if ((zip.x = (word8 *)mem_Malloc(buffer->length * 2)) == NULL)
			exit(0);
		//分配结束
		...
		if ((zero.x = (word8 *)mem_Malloc(zero.length)) == NULL)
			exit(0);
...
int RAYCompression(DATA *buffer)
{
	...
	if ((stat = (unsigned int *)mem_Malloc((buffer->length)*8)) == NULL)
	     	return -2;
	if ((zip.x = (BYTE *)mem_Malloc((buffer->length )*2)) == NULL)
	{
		mem_Free(stat);// add by tanxin 2006.9.7
		stat = NULL;
		return -2;
	}
	if ((buffer_copy.x = (BYTE *)mem_Malloc(buffer->length)) == NULL)
	{
		mem_Free(stat);
	    mem_Free(zip.x); // add by tanxin 2006.9.7
		stat = NULL;
		zip.x = NULL;
		return -2;
	}
...
int SHA_64(DATA * buffer)
{
	...
	if ((c = (word8 *)mem_Malloc(l+128)) == NULL)
		exit(0);
	memset(c,0,(l+128));
...
int ExpendPeriod(DATA *buffer)
{
	...
	if (buffer->x[0] == BIAOSHI1)
	{
		...
		if ((recover.x = (word8 *)mem_Malloc(data_length)) == NULL)
			exit(0);
		//分配结束
...
int ExpendRAY(DATA *buffer)
{
	...
	{
		if ((recover.x = (BYTE *)mem_Malloc(data_length*2)) == NULL)
			return -2;
		recover.length = 0;
...
int SHA_1(DATA *buffer)
{
	...
	if ((c = (BYTE *)mem_Malloc(l+1024)) == NULL)
		return -2;
	memset(c,0,(l+1024));
