...
sys_res arch_SpiConfig(spi_t *p, int mode, int order, int speed)
{
	SPI_TypeDef *spi;
	...
	spiinit.BitOrder = order ? LL_SPI_MSB_FIRST : LL_SPI_LSB_FIRST;
...
sys_res arch_SpiTransThenRecv(spi_t *p, const void *send, size_t slen, void *rec, size_t rlen)
{
	SPI_TypeDef *spi = stm32_tbl_spiid[p->def->id];
	const u8 *tp = (const u8 *)send;
	u8 *tr = (u8 *)rec;
	__IO u32 temp;
...
sys_res arch_SpiTransmit(spi_t *p, const void *send, void *rec, size_t len)
{
	SPI_TypeDef *spi = stm32_tbl_spiid[p->def->id];
	const u8 *tp = (const u8 *)send;
	u8 *tr = (u8 *)rec;
